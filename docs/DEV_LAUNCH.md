...
**Last verified:** 2026-09-17 — restore/build/test/CLI-run all pass from a clean state on **Ubuntu 24.04** (.NET SDK 8.0.131), Phase 6c.6 (6C completion gate): Release build 0 warnings/0 errors, full suite **300 green** (Core 85, Data 58, Cli 35, App 122) including the 6c.6 widget-selector / empty-state / NFR-6 verification tests, real Linux launch 18 s alive with "Main window created." and crash logs unchanged; headless evidence — `logs/screenshots/phase-6c-input-analysis.png` (all four Input Analysis histogram + chi-square cards from a real file load), `logs/screenshots/phase-6c-results-all.png` (all seven Results widgets in one frame from a real 3-stage run) and `logs/screenshots/phase-6c-widget-toggled.png` (widget picker open, one widget switched off), plus the earlier 6c.1–6c.5 PNGs (`phase-6c1-empty.png`, `phase-6c2-histograms.png`, `phase-6c3-chi-square.png`, `phase-6c4-utilisation.png`, `phase-6c5-charts.png`). M1 headless CLI verified: stable run (ρ 0.75) and clean unstable refusal (single-line stderr, no stack trace, exit 1 — ρ 1.25). M2 data CLI verified: `verify` (clean file → exit 0; dirty fixture → exit 1 listing all 5 issues), `fit` (prints params + chi-square, writes `logs/fit-*.json`), `simulate-data --servers 1,2,3` (three runs, exit 0), `export`. M3 `simulate-network` verified (incl. `--days 5 --cap 80 --verbose`). M4 `trace` verified against the frozen golden fixture (state/rng/events; `--output`; unstable refusal). **M5/Rebuild GUI verified: ava headless renders of MainWindow (phase-1 + controls-demo PNGs in `logs/screenshots/`); real-display launch/keyboard walk is owner-required on a machine with a display (this host is Wayland).** [Windows: TBD]
...
Expected: `Passed! - Failed: 0`. As of 2026-09-17 **300 tests pass**:
...
- `OpdSimulator.App.Tests` (122, headless Avalonia, GUI rebuild Phase 1–4, 4b, 4c, 5, 5c, 5c.4, 5d, 6c.1, 6c.2, 6c.3, 6c.4, 6c.5, 6c.6) — Avalonia.Headless
