...
- [~] In-progress: PROJECT_DEEP_DIVE.md (viva-preparation document, 18 chapters, 5 sessions on `docs/deep-dive-ch1-6`) — **Session A (ch. 1–6, cover/exec-summary/architecture/repo-map/packages/build) DONE 2026-09-16**, committed as `docs: deep dive, chapters 1–6`; **Session B (ch. 7–9, Core/Data/Cli) DONE 2026-09-16**, committed as `docs: deep dive, chapters 7-9`; **Session C (ch. 10–11, App/Tests) DONE 2026-09-16**, committed as `docs: deep dive, chapters 10-11`; **Session D (ch. 12–15, workflows/algorithms/invariants/decision summary) DONE 2026-09-16**, committed as `docs: deep dive, chapters 12-15`; **Session E (ch. 16–18, viva bank/limitations/glossary) + PDF awaits owner "go"** per the 5-session plan. PDF via wkhtmltopdf (pandoc/xelatex/weasyprint absent) in Session E; `docs/DEEP_DIVE_BUILD.md` will record the working command.
...
- [~] **Phase 6C — Input Analysis tab + chart suite** (branch `feat/milestone-6c-input-analysis-charts`, owner "go" 2026-09-16). Replaces the Input Analysis placeholder + former "M5 charts" rows: P1 input charts (histogram + fitted PDF, chi-square O/E, per-server utilisation FLAG) and P2 output charts (queue-length over time, waiting-time histogram) — FR-UI-4, FR-STAT-8, NFR-6. **Charts never touch Core**: pre-flight confirmed `StageMetrics.WaitingTimeSamples` + `QueueLengthSeries` already exist (c4fe7ee) — the 6c.5 "minimal Core addition" step is permanently dropped per owner instruction. Gated: STOP + owner "go" at every 6c.x boundary; each sub-phase commits + pushes its own conventional message.
  ...
  - [ ] 6c.4 — Per-server utilisation bar chart with imbalance flag (threshold 0.15, amber `BrushWarning`, tooltip "Above average by {delta:.1%}"); `logs/screenshots/phase-6c4-utilisation.png`.
  - [ ] 6c.5 — Queue-length over time (current 4-series colour palette, downsampled ≤ 2000 pts) + waiting-time histogram from `StageMetrics`; `logs/screenshots/phase-6c5-queue.png` + `phase-6c5-wait.png`. NO Core change.
  - [ ] 6c.6 — Completion gate: docs (DECISIONS, USER_MANUAL "Reading the Charts", VIVA_ANSWERS, DEV_LAUNCH, CONTEXT), REQUIREMENTS rows FR-UI-4/FR-STAT-8/NFR-6, full suite, all screenshots (`phase-6c6-input-analysis.png`, `phase-6c-results-all.png`, `phase-6c-widget-toggled.png`), STOP for final review + merge.
