...
- [ ] **Phase 5d — config panel refinements** — 2026-09-16 (5d.1 μ moves out of Stages — read-only μ-source label, single manual entry point in Parameters, Start gating on missing μ with ErrorBanner — D-112; 5d.2 α selector in Model with (0,1) validation + dynamic chi-square caption — D-113; 5d.3 stage-count mismatch amber warning + "Sync stages from data" — D-114, warning tokens D-115). Not started — 5c.4 done first (IDs renumbered per D-109 note: 5d is now D-112..D-115).
