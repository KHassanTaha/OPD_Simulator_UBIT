...
- [~] **Phase 5c — post-review bug fixes** — 2026-09-16, 03:20:
  - 5c.1 **DONE** — widgets in a ScrollViewer (Vertical=Auto, Horizontal=Disabled), customise toggle + widget picker pinned above it (ResultsPanel.axaml `RowDefinitions="Auto,*"`); results column `380,6,*` with `MinWidth=540` on the Border because Avalonia's compact grid parser rejects `MinMax(540,*)` (AVLN2005 — logged in the code comment, D-107 covers the quirk filter, the MinLength realisation belongs with 5c.1). Test `ResultsPanel_ScrollViewer_ContainsAllWidgets`; screenshot `logs/screenshots/phase-5c-results.png`.
  - 5c.2 **DONE** — TaskScheduler handler ignores the known Wayland `AppMenu.Registrar` DBus quirk (D-107): walk inner chain, match marker + `org.freedesktop.DBus.Error.ServiceUnknown` (`Tmds.DBus.Protocol.DBusException` carries it in the message — verified against the real 02:49 crash log), SetObserved + Information log, no dialog. Test `CrashReporter_IgnoresAppMenuRegistrarDBusError`.
  - Discovered+fixed (5c-in-scope): **FR-UI-14 restore gap** — `MainViewModel` built `new WidgetPreferences()` (never `Load()`), so persisted visibility was never restored, and the `WidgetPreferences` default seed was all-OFF, contradicting the documented all-on. D-108. The machine's `ui.json` (`VisibleWidgets: []`, written 03:08 during tests) is the visible symptom — flagged to the owner, not deleted.
  - Discovered+fixed (5c part 3, owner-reported): **welcome card never rendered** — `ResultsPanel`'s ContentControl bound only `IsVisible`, never `Content="{Binding Welcome}"`, so the ContentTemplate never instantiated `WelcomeCard` (D-109). `WelcomeCard.axaml` existed and was complete (AGENTS §16.6). Test `WelcomeCard_VisibleOnFreshLaunch_AndHiddenAfterRun`; commit `fix: welcome card visible on fresh launch (Phase 5c, part 3)`.
  - 5c.3 trace in ALL run modes — **BLOCKED [~?]** on the frozen-Core rule: the calendar `Engine.Run` overload (Engine.cs:184) has no `traceSink` parameter and hardcodes `traceSink: null` (line 198), so the App cannot wire a sink for ClinicDay/MultiDay without a minimal Core change. Needs owner decision — **B-008**.
  - Gate evidence so far: Release build 0/0, full suite **233 green** (App +3 = 55), real launch smoke 15 s alive "Main window created." + crash log unchanged (3:18 run), screenshot 65 KB rendered (metrics + chi-square + populated State trace visible, overflow asserted in-test).
  - Pending: commit (owner-specified message covers 5c.3 which is NOT done — ask at checkpoint), push, then Phase 5d.
- [ ] **Phase 5d — config panel refinements** — 2026-09-16 (5d.1 μ moves out of Stages — read-only μ-source label, single manual entry point in Parameters, Start gating on missing μ with ErrorBanner — D-111; 5d.2 α selector in Model with (0,1) validation + dynamic chi-square caption — D-112; 5d.3 stage-count mismatch amber warning + "Sync stages from data" — D-113, warning tokens D-114). Not started — 5c checkpoint first (IDs renumbered per D-109 note).
...
- Manual service-rate inputs are duplicated: the per-stage `ServiceRate` field is authoritative and Parameters' `ManualMuPerStage` list only backfills blanks (D-106) — consolidate into one input path (e.g., keep per-stage rows, drop the list).
