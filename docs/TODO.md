...
- [ ] **Phase 5d — config panel refinements** — 2026-09-16 (5d.1 μ moves out of Stages — read-only μ-source label, single manual entry point in Parameters, Start gating on missing μ with ErrorBanner — D-109; 5d.2 α selector in Model with (0,1) validation + dynamic chi-square caption — D-110; 5d.3 stage-count mismatch amber warning + "Sync stages from data" — D-111, warning tokens D-112). Not started — 5c checkpoint first.
