...
- [~] In-progress: PROJECT_DEEP_DIVE.md (viva-preparation document, 18 chapters, 5 sessions on `docs/deep-dive-ch1-6`) — **Session A (ch. 1–6, cover/exec-summary/architecture/repo-map/packages/build) DONE 2026-09-16**, committed as `docs: deep dive, chapters 1–6`; Sessions B–E await owner "go" per the 5-session plan in the task brief. PDF via wkhtmltopdf (pandoc/xelatex/weasyprint absent) in Session E; `docs/DEEP_DIVE_BUILD.md` will record the working command.
