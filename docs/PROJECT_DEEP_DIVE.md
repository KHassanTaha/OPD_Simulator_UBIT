...
| D | 12–15 (workflows, algorithms, invariants, decision summary) | **written** 2026-09-16 |
...
# 12. Workflows — the Journeys to Demo

## 12.0 The three journeys

The project offers three demo-able journeys. Keep this frame ready: every
question in the viva maps onto one of them.

| # | Journey | Entry point | Milestone | What it proves |
|---|---------|-------------|-----------|----------------|
| 1 | Parameter-driven model | `simulate-params`, `simulate-network` | M1, M3 | queueing theory, ρ, stability |
| 2 | Data-to-decision | `verify` → `fit` → `simulate-data` → `trace` | M2, M4 | loading, fitting, chi-square, DES |
| 3 | Point-and-click GUI | Avalonia app: Upload → Analyse → Run | M5, 6c | usability, threading, charts |

## 12.1 Journey 1 — the CLI, parameter-driven

The CLI dispatches on the first argument
(`Program.cs:95-102`); the usage text doubles as the command reference
(`Program.cs:30-41`).

| Step | Command | What happens | Cite |
|------|---------|--------------|------|
| Model one stage | `dotnet run --project src/OpdSimulator.Cli -- simulate-params --help` | M/M/c from rate parameters | `Program.cs:33` |
| Model the network | `simulate-network --days 3 --verbose` | three-stage run with the clinic calendar; `--days` selects calendar mode, `--verbose` prints the pre-run ρᵢ | `Program.cs:37`, D-053 |
| Read the result | exit code + metrics line | `0` success, `1` refused (ρ ≥ 1 or dirty data), `2` usage error; a refusal prints one clean line to stderr (D-037), the stack trace only to `logs/` | `Program.cs:17-24`, ch. 9 |

Viva phrasing: *"simulate-params validates the engine against analytical
M/M/c (FR-STAT-5); simulate-network is the three-stage clinic with the
calendar and routing."*

## 12.2 Journey 2 — the CLI, data-driven

| Step | Command | What happens | Cite |
|------|---------|--------------|------|
| 1. Validate | `verify <file>` | load + validate; every issue printed; dirty rows named, not silently dropped | `Program.cs:34` |
| 2. Fit | `fit <file> --col ...` | MLE fit + chi-square; writes `logs/fit-*.json` for SPSS cross-check | `Program.cs:35`, ch. 8 |
| 3. Simulate | `simulate-data <file>` | single stage: `--servers` sweep; stage-aware: per-stage server counts with clinic-flow ordering | `Program.cs:36`, D-052 |
| 4. Trace | `trace <file>` | deterministic, line-by-line event trace — the M4 viva artefact | `Program.cs:38`, D-055…D-059 |

The synthetic patient, its fixture and the sample file are all generated by a
standalone tool, not hand-written (D-047, D-045); wall-clock times are stored
at second precision (D-048).

## 12.3 Journey 3 — the GUI

1. **Launch** — `dotnet run --project src/OpdSimulator.App` (`Program.cs:19`).
   The welcome card fills the results column until a run starts (D-063,
   D-109).
2. **Configure** — Upload button and the D-114 mismatch strip (ConfigPanel
   Section 1), distribution dropdowns + α selector (Section 2), λ + single μ
   comma list + p_exit override (Section 3), per-stage server counts with
   topology-only rows (Section 4, D-112), optional Sections 5–6 via the
   D-102 enable toggle.
3. **Run** — the pinned Start button; the run moves to a background thread
   with a cancellation token (`MainViewModel.cs:106`); the results panel swaps
   placeholders for live values (`ResultsPanelViewModel.cs:73-111`).
4. **Inspect** — the Input Analysis tab (6c.2) shows histograms, fitted PDF
   and chi-square cards fed by the same `ChiSquareResult` arrays D-118,
   D-120; the Simulation tab shows metrics, per-stage ρᵢ and the scrollable
   trace.

## 12.4 The diagnostics journey

- `logs/app-YYYYMMDD.log` — structured events (Serilog).
- `logs/errors-YYYYMMDD.log` — error-only.
- `logs/crash-YYYYMMDD.log` — the three global exception handlers write full
  stack traces here after a dialog (App `CrashReporter`, ch. 10.1).
- `trace` command stdout is pure trace lines (D-059) and is byte-stable
  across platforms for a fixed seed.

Viva line: *"If behaviour is surprising I read the crash log first, then the
error log, then the app log — in that order"* (AGENTS §12.4).

## 12.5 The demo-day sequence (the one to rehearse)

Per D-011 the showcase has been the CLI path, and the GUI rebuild (phases
1–5d, 6c.1–6c.3) is the current arc. A tight 5-minute sequence:

1. `verify samples/sample_patients.csv` — clean sheet.
2. `fit` — show λ̂, μ̂, chi-square verdict + `logs/fit-*.json`.
3. `simulate-network --days 2 --verbose` — ρᵢ printed, stable, metrics out.
4. Open the app, upload the same file, Start, show the metrics + ρᵢ matching.
5. `trace` — hand-walk one patient through Reception → Screening → exit.

> **Viva box (12.5).** The professor may ask "why does the GUI matter if the
> CLI is complete?" Answer: the CLI is the testable core; the GUI is the
> deliverable that lets a non-technical clinic manager run the same engine
> without reading a trace file — same code path, different front end.

---

# 13. Algorithms — Show Your Working

This chapter is the mathematical skeleton. Exams and vivas probe it line by
line; every section points at the exact source line.

## 13.1 The discrete-event loop

The engine is a textbook FEL loop (`Engine.cs:19-20` documents the loop in
its prologue). Concretely:

1. **Seed** — the deterministic RNG is seeded; the seed is traced as an RNG
   row (`Engine.cs:225`).
2. **First arrival at t = 0** — `fel.Enqueue(new Event(0, EventType.Arrival,
   nextPatientId))` (`Engine.cs:265`).
3. **Loop** — `while (fel.Count > 0)` pop the earliest event, advance the
   clock to its time, dispatch on `EventType` (`Engine.cs:267`, switch at
   `Engine.cs:293-317`).
4. **Dispatch** — `Arrival` / `ReceptionEnd` / `ScreeningEnd` /
   `DoctorEnd`; the end events are generated by
   `EndEventTypeForStage(stageIndex) => (EventType)(stageIndex + 1)`
   (`Engine.cs:564-567`) — see invariant 14.1.2.

**Tie-breaking.** Two events due at the same time cannot be ordered by clock
alone, so `Event.CompareTo` orders by **time, then event type, then patient
id** (`Event.cs:44-57`, D-033). Event type therefore doubles as a priority
class: an Arrival happens before an End event at the same clock, which keeps
arrivals and completions in the right order.

| Event step | Handled by | Line |
|-----------|-----------|------|
| Arrival | `HandleArrival` | `Engine.cs:377-435` |
| Service start | `StartService` | `Engine.cs:439-467` |
| Completion | `HandleServiceEnd` | `Engine.cs:470-514` |
| Routing | `RouteToNextStage` | `Engine.cs:531-561` |

## 13.2 Arrivals and the calendar gate

`HandleArrival` (`Engine.cs:377-435`):

1. If the arrival lands outside the open window **or** the daily cap is
   reached, the patient is **not admitted** — trace says so explicitly
   (`Engine.cs:413-419`).
2. The next inter-arrival time is drawn from the exponential sampler
   (`Engine.cs:425`) and the next arrival is scheduled **only if it falls
   inside the window** (`Engine.cs:420-435`): `stopTime` is the gate's
   stop time in calendar mode, else `horizonMinutes`.
3. The draw is emitted as an RNG trace row so the calendar's stream is
   observable (FR-VAL-4).

The **CalendarGate** (`Engine.cs:617-709`) implements D-051: a **gated
Poisson stream** — arrivals continue to be *drawn* at the exponential rate
but only *fired* inside the open window, and a run continues past 11:00 to
drain in-progress service (FR-SIM-6). It tracks per-day first arrival and
last service end, which define the **operating time** denominator (D-018),
and the admitted count per day for the cap (`Engine.cs:659-709`).

## 13.3 Service, waits and histograms

`StartService` (`Engine.cs:439-467`):

1. `patient.MarkServiceStarted(clock)`; the server is marked busy and the
   patient–server map is populated (`Engine.cs:444-447`).
2. **Wait at that stage** = `clock − patient.ArrivalTime` (stage arrival
   time), added to the running total and pushed onto the waiting-time sample
   series (`Engine.cs:452-455`).
3. Service time = `_serviceSampler.Sample(stage.ServiceRate)`, traced with
   the exact end time (`Engine.cs:454-461`), and the End event is scheduled
   at `clock + serviceTime` (`Engine.cs:467`).

The queue-length trace reflects the count **after** the dequeue, so every
row reconstructs the exact state at that clock.

## 13.4 Routing and early exit

`RouteToNextStage` (`Engine.cs:531-561`):

- If the patient exits after Screening (probability `p_exit`), an exit row
  is traced and the patient leaves (`Engine.cs:514`).
- Otherwise they are placed in the next stage's queue and a Route row is
  traced with the queue length after insertion (`Engine.cs:561`).

Effective rates by stage use the routing probabilities (D-015,
`NetworkTopology.EffectiveArrivalRate`, `NetworkTopology.cs:72`).

## 13.5 MLE fitting

The exponential fit is MLE: λ̂ = 1 / (sample mean), which is both the
maximum-likelihood and the method-of-moments estimator. Other distributions
are fitted via MathNet.Numerics — gamma by method-of-moments (D-041),
normal/lognormal σ with the MLE `n` denominator (D-043). The fitted
distribution carries CDF/InverseCDF delegates so chi-square and sampling
share one implementation (D-042). Details and citations: ch. 8.

## 13.6 Chi-square goodness-of-fit — step by step

Implementation: `Fitting/ChiSquareTest.cs` (ch. 8 cites; algorithm lines
below).

1. **Choose k bins** via the square-root rule (`BinSelector.BinCount`,
   `ChiSquareTest.cs:43`), edges by equal probability from the fitted CDF
   (`ChiSquareTest.cs:44`, D-040).
2. **Expected per bin** E = n / k (`ChiSquareTest.cs:47`).
3. **Guard:** if E < 1 the test refuses to claim a verdict
   (`ChiSquareTest.cs:50`, D-044).
4. **Degrees of freedom** df = k − 1 − p (p fitted parameters); a guard
   rejects df < 1 (`ChiSquareTest.cs:60-63`).
5. **p-value** = 1 − CDF_χ²(df)(statistic) — MathNet
   (`ChiSquareTest.cs:65`).

Worked arithmetic (illustrative): 100 samples, k = 10 bins → E = 10 each; if
the fitted exponential says almost all mass lies in bin 1, bin 1 contributes
((O₁−10)²)/10 while bins far out contribute E ≈ 10 with O ≈ 0 → statistic
grows → p-value falls below α → verdict "reject". The honest caveat to state
aloud: *fail to reject ≠ the distribution is correct; report the p-value,
not just the verdict* (`docs/CONTEXT.md:103-106`).

## 13.7 RNG determinism

`SeededRandom` wraps `System.Random` with a fixed seed (default 42) and an
inverse-CDF exponential generator (D-035); the trace **observes** the RNG
stream but never perturbs it (D-057) — proof of determinism:

    seed 42 → same inter-arrival sequence → same trace bytes, every platform.

## 13.8 The clinic calendar

Closed days are skipped entirely; Saturday end steps to Monday t = 0 with no
activity (D-051, `docs/CONTEXT.md:130-132`). Operating time runs from the
day's first arrival to its last service end (D-018); the DailyCap is an
admission bound (per-engine noted in ch. 7). The UI's start-day anchor and
daily-cap fields map 1:1 onto these core types.

## 13.9 Stability refusal

Per stage, ρᵢ = λᵢ / (cᵢ·μᵢ) (`RhoFor`, `NetworkTopology.cs:88-91`) is
derived from routing-propagated arrival rates (`NetworkTopology.cs:72`). If
any ρᵢ ≥ 1 the topology refuses to validate (D-007, D-034,
`NetworkTopology.cs:114-125`) and the run is refused with a clean stderr
line (D-037). Constructor guards enforce λ > 0, ≥ 1 stage, p_exit ∈ [0, 1)
and a valid exit stage (`NetworkTopology.cs:41-47`).

## 13.10 Take-away

> **Viva box (13.10).** "Trace one event's full journey by hand." Say:
> arrival at t — draws inter-arrival and service times — the End event carries
> `EventType = stageIndex + 1` — on completion, wait accumulates, routing
> decides exit vs doctor with p_exit — every decision is a traced row. Then
> point at `Engine.cs:265-467` as you speak.

---

# 14. Invariants — Rules the System Never Breaks

An invariant is a property that holds at **every** instant if the system is
correct. When a test asserts one, it verifies intent, not just output.

## 14.1 Engine invariants

| # | Invariant | Where enforced | Test evidence |
|---|-----------|----------------|---------------|
| 14.1.1 | The clock never moves backwards — each popped event has time ≥ the previous pop | FEL pop + `Event.CompareTo` (`Event.cs:44-57`) | Engine tests, trace deletion tests |
| 14.1.2 | `EventType == stageIndex + 1` for end events | `EndEventTypeForStage` (`Engine.cs:564-567`) | Engine phase-4 tests |
| 14.1.3 | FEL ties break time → type → patient id | `Event.cs:49-57`, D-033 | `FelTests` |
| 14.1.4 | p_exit ∈ [0, 1), λ > 0, ≥ 1 stage | `NetworkTopology.cs:41-47` | topology tests |
| 14.1.5 | ρᵢ ≥ 1 at any stage ⇒ run refused | `RhoFor` + `Validate` (`NetworkTopology.cs:88-125`), D-034 | refusal tests in Core + Cli |
| 14.1.6 | New arrivals are scheduled only inside the open window or before the horizon | `HandleArrival` (`Engine.cs:413-435`) | `Run_Calendar_*` tests |
| 14.1.7 | A service end is always scheduled at `start + serviceTime` ≥ start | `StartService` (`Engine.cs:467`) | trace regression |
| 14.1.8 | Simulated utilisation ∈ [0, 1] | utilisation = busy time / operating time, D-016 | metrics tests |
| 14.1.9 | Daily admissions ≤ DailyCap when cap set | `CalendarGate.AdmittedToday` (`Engine.cs:659-709`) | cap tests |

## 14.2 Statistics and trace invariants

| # | Invariant | Where enforced | Test evidence |
|---|-----------|----------------|---------------|
| 14.2.1 | The trace tells the same story as the statistics job | trace rows + stage metrics cross-checked | D-058, trace/stats cross tests |
| 14.2.2 | The trace preserves the exact queue semantics, column by column | trace schema (five patient rows + one RNG row), D-056 | TraceRegression |
| 14.2.3 | The trace observes, never perturbs, the RNG stream | `TraceRandomSource` passive wrapper, D-057 | seed-stability tests |
| 14.2.4 | `trace` stdout contains only trace lines | file-only logger, D-059 | CliTrace tests |
| 14.2.5 | Waiting time at a stage = start − stage arrival | `Engine.cs:452` | statistics tests |

## 14.3 Data and fitting invariants

| # | Invariant | Where enforced |
|---|-----------|----------------|
| 14.3.1 | λ₀ computed only when ≥ 2 arrivals exist | `DataAnalyzer.cs:49-58` |
| 14.3.2 | `departure_stage = Reception` excludes the row from p_exit (anomaly, D-008) | validator, ch. 8 |
| 14.3.3 | E ≥ 1 and df ≥ 1 before a chi-square verdict | `ChiSquareTest.cs:50,60-63`, D-044 |
| 14.3.4 | Equal-probability bins, k from `√n` (clamped) | `ChiSquareTest.cs:43-47`, D-040 |
| 14.3.5 | Rate↔mean conversion happens once, before the engine | `ConfigPanelViewModel.cs:917-918` |

## 14.4 GUI invariants

| # | Invariant | Where enforced |
|---|-----------|----------------|
| 14.4.1 | The welcome card disappears only when a run starts (never because a preset exists) | D-063, D-109, `MainViewModel` |
| 14.4.2 | A field in error shows red + icon + message — never colour alone | FR-UI-17, D-069 |
| 14.4.3 | The results panel holds placeholders until a run produces values | `ResultsPanelViewModel.cs:73-111` |
| 14.4.4 | All colours/fonts/spacing come from Theme.axaml (85 tokens) — hex never inline | D-062, ch. 10.7 |
| 14.4.5 | p_exit is hidden for 1-stage configurations | D-097 |
| 14.4.6 | "Clear All" returns to the fresh-launch state (full reset) | D-111 |

## 14.5 The reproducibility invariant

A fixed seed yields a byte-identical trace on any platform and any run —
this is the single most examinable invariant. It is what makes M4's trace a
valid evidence artefact and it is protected by seed-gating fixes (D-106) and
the D-057 wrapper.

> **Viva box (14.5).** "Which invariant would break first if someone
> 'improved' the FEL with a different container?" — The tie-break order
> (14.1.3). A `<` reversal silently changes which patient is served first at
> an equal clock, so the trace bytes change while *looking* correct. That is
> why a hash of the golden trace is asserted in tests, not just the metrics.

---

# 15. Decision Summary — D-001 → D-120

Every decision the project made is logged with date, rationale, impact and
alternatives in `docs/DECISIONS.md` (§9.2 requires an entry at the moment a
decision is made). This chapter is the reverse index: grouped by concern,
each with the chapter where it is used.

## 15.1 Stack and tooling

| Decision | Gist | Used in |
|----------|------|---------|
| D-001 | C# on .NET 8 LTS | ch. 6 |
| D-002 | Avalonia UI over MAUI/Uno | ch. 10 |
| D-003 | MathNet.Numerics for statistics | ch. 8 |
| D-004 | ClosedXML + CsvHelper for data I/O | ch. 8 |
| D-005 | xUnit | ch. 11 |
| D-013 | `global.json` pins SDK 8.0.100, `rollForward: latestFeature` | ch. 6 |
| D-026 | NuGet versions pinned at scaffold | ch. 5 |
| D-078 | App scaffolded by hand, not `dotnet new` | ch. 10 |
| D-079 | Avalonia needs native X11/fontconfig on Linux | ch. 6, 10 |
| D-116 | LiveChartsCore.SkiaSharpView.Avalonia 2.0.5 re-pinned for charts | ch. 10.7 |
...
| D-049 | Engine runs a `NetworkTopology`; the single-stage path delegates |
| D-085 | Computed VM booleans instead of value converters |
| D-086 | GUI rebuild disposed the old view layer, rebuilt on `feat/gui-rebuild` |
| D-094 | Shell: four-tab TabControl + 380 px config / fill results |
| D-095 | Tab content lives in the TabControl ContentPresenter |

## 15.3 Docs, process, git, CI

D-010, D-011, D-012 (living docs, demo scope, AGENTS/DEV_LAUNCH/USER_MANUAL
locations), D-020 (trunk-based + conventional commits + CI), D-021 (Serilog
+ global handlers), D-022/23 (session wrap-up / resume protocol), D-024/25
(REQUIREMENTS.md traceability + bidirectional rule), D-027/29
(`appsettings.template.json`), D-028/31 (root commit on main), D-030 (one
canonical location for instructions), D-032 (reconciliation gated before
M1), D-047 (sample + fixture generated by a tool, not hand-written), D-045
(sample CSV tracked via .gitignore negation).

## 15.4 Engine and queueing

| Decision | Gist |
|----------|------|
| D-007 / D-015 | ρ uses effective per-stage arrival rate via routing |
| D-016 | Per-server utilisation always available |
| D-017 / D-050 | Server assignment: random among idle (M1 lowest-id bias fixed) |
| D-018 | Operating time = first arrival → last service end |
| D-033 | FEL ordering: time → event type → patient id |
| D-034 | UnstableSystemException replaces silent unbounded growth |
| D-035 | Deterministic RNG + inverse-CDF exponential |
| D-036 | Engine event trace at Serilog debug |
| D-051 | Clinic calendar: gated Poisson stream, per-open-day operating time, DailyCap |
| D-052 | Stage-aware `simulate-data`: clinic-flow ordering, blank-downstream rule, per-stage counts |
| D-110 | Calendar `Engine.Run` forwards an optional `traceSink` |

## 15.5 Statistics

| Decision | Gist |
|----------|------|
| D-038 | M2 validation stricter than CONTEXT §5.4 on `departure_stage` |
| D-040 | Chi-square: equal-probability bins, `√n` bins, df = k − 1 − p |
| D-041 | Gamma fit by method-of-moments |
...
D-008 (`departure_stage = Reception` = anomaly), D-014 (no synthetic sample
data initially), D-039 (TimeParser formats + bare-number heuristic), D-048
(second-precision wall-clock storage), D-053 (network flags, not JSON),
D-054 (normalised single-stage output; metrics are the contract).

## 15.7 CLI and trace

D-037 (clean refusal line; stack trace only in logs), D-046 (subcommand
dispatcher), D-055 (trace is a first-class sink), D-056 (trace schema: five
patient rows + one RNG row), D-057 (trace observes, never changes RNG),
D-058 (trace and statistics cross-checked by test), D-059 (`trace` stdout is
pure trace lines).

## 15.8 GUI M5 requirements batch

D-060 (welcome panel content) · D-061 (searchable dropdown control) ·
D-062 (single theme dictionary) · D-063 (welcome card dismissal) · D-064
(results panel customisation) · D-065 (keyboard navigation as contract) ·
D-066 (label + placeholder + tooltip + accessible name) · D-067
(ValidatedField) · D-068 (validate on blur) · D-069 (red + icon + message) ·
D-070 (preview read-only) · D-071 (preview virtualisation) · D-072 (guide
renders embedded markdown) · D-073 (presets under ApplicationData) · D-074
(versioned schema) · D-075 (startup starts empty) · D-076 (preset dropdown
default).

## 15.9 GUI rebuild phases 1–5d

D-077 (PNG logos not SVG) · D-080 (ContentControl themes vs UserControl
composites) · D-081 (virtualising ListBox over ItemsRepeater) · D-082
(hand-rolled guide parser over Markdig) · D-083 (preset schema v1,
`arrivalParameter`, no auto-restore) · D-084 (chart resource colours via
`Application.Resources` fallback factory) · D-087 (Phase-1 token contract:
Theme.axaml + Motion.axaml) · D-088 (M5 Serilog 3 sinks + CrashReporter
kept) · D-089 (headless frame capture for screenshots, Wayland constraint) ·
D-090 (DataPreviewTable virtualisation) · D-091 (`OnApplyTemplate` +
`INameScope.Find` wiring) · D-092 (compiled-binding `$parent` rule) · D-093
(self-hidden ErrorBanner is a dead control) · D-096 (one ScrollViewer +
PinnedFooterBar) · D-097 (p_exit hidden for 1 stage) · D-098 (p_exit ∈ [0, 1)
matches Core) · D-099 (stage row = view-model class) · D-100 (blank μ = use
fitted) · D-101 (CollapsibleSection header redesign) · D-102 (optional-
section enable toggle) · D-103 (OFF sections skip Start gating).

## 15.10 Phase-5 coordinator, 5c/5d

D-104 (coordinator re-implements the M5 data/run seam in-app) · D-105
("Diagnostic trace" run mode) · D-106 (manual-μ source resolution + seed
gating fix) · D-107 (Wayland `AppMenu.Registrar` DBus quirk ignored) · D-108
(widget visibility persistence corrected to all-on defaults) · D-109 (welcome
card bound as Content, not IsVisible) · D-111 ("Clear All" = full reset) ·
D-112 (μ leaves Stage rows; one manual entry point) · D-113 (α selector with
(0,1) validation) · D-114 (stage-count mismatch amber warning + Sync/Keep) ·
D-115 (amber theme tokens + `ErrorBanner.Severity`).

## 15.11 Input Analysis 6c

D-117 (chart series palette via `ChartTheme.axaml` brush contract, hex never
inline) · D-118 (histogram reuses the chi-square result's bins) · D-119
(explicit binding-change event + background fit superseding the generation
guard) · D-120 (chi-square cards reuse the verdict's bins, two side-by-side
columns).

## 15.12 The cross-cutting themes

- **Correctness by contract:** invariants are enforced at the type boundary
  (topology ctor, chi-square guards, FEL ordering), not by convention.
- **Evidence by trace:** every random draw and state change is a trace row,
  so any claim can be replayed (trace + cross-checks D-056…D-059, D-058).
- **GUI discipline:** one theme dictionary, reusable controls, validation
  patterns, keyboard contract — assigned line-by-line in §16.2 of AGENTS.

> **Viva box (15.12).** "Pick one decision and defend it." Best picks:
> D-018 (why not the fixed 8:15–11:00 denominator — closing-time overtime
> would push utilisation toward/past 1), D-033 (why event-type in the tie-
> break), or D-098 (why p_exit is [0, 1) and p_exit = 1 is rejected as a
> data-meaning case, not a crash).

**Chapters 12–15 verified against:** the DES loop and calendar gate
(`Engine.cs:265-467, 564-567, 617-709`), event ordering (`Event.cs:44-57`),
topology guards and stability (`NetworkTopology.cs:41-125`), arrival
scheduling and operating time (`Engine.cs:413-435`), chi-square algorithm
(`Fitting/ChiSquareTest.cs:43-74`), CLI command inventory and exit codes
(`Program.cs:17-41, 95-102`), GUI run flow (`MainViewModel.cs:106`,
`ResultsPanelViewModel.cs:73-111`), and every entry of `docs/DECISIONS.md`
(D-001 … D-120, 120 decisions). All line numbers re-read this session;
decision headings re-grepped 2026-09-16.
