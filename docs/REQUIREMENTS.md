...
| FR-UI-4 | Charts via LiveCharts2 (P1 + P2) | [~] | Views/InputAnalysisView.axaml, ViewModels/InputAnalysisViewModel.cs, ViewModels/InputAnalysisChartViewModel.cs, Services/InputAnalysisService.cs, Services/ChartControlBuilder.cs (6c.2 = input histogram cards + fitted-PDF overlay; 6c.3 = per-fit chi-square observed-vs-expected bars with verbatim ResultsPanel caption; 6c.4 = per-server utilisation widget on the Results tab, run-driven per D-121; P2 queue/wait charts follow in 6c.5) | Phase6c2HistogramTests + Phase6c3ChiSquareTests + Phase6c4UtilisationTests + Phase6c4Screenshot (App.Tests) | D-118, D-119, D-120, D-121 |
