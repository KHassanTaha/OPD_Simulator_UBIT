...
public static class SimulationCoordinator
{
    ...
    /// <summary>Banner shown when a stage has neither a manual nor a fitted service rate.</summary>
    public const string MissingServiceRatePrefix = "Stage has no service rate: enter one in Stages, or load data with timings for it.";
    ...
    public static RunOutcome Run(SimulationParameters parameters, DataBindingResult? binding, Action<string>? status = null)
    {
        ...
        var specs = BuildStageSpecs(parameters, binding);
        if (specs is null)
        {
            return Refused(fits, exitProbability, MissingServiceRatePrefix);
        }
        ...
        try
        {
            var topology = new NetworkTopology(arrivalRate.Value, specs, exitStageIndex, exitProbability);
    ...
    /// configured.
    /// </summary>
    private static List<StageSpec>? BuildStageSpecs(SimulationParameters parameters, DataBindingResult? binding)
    {
